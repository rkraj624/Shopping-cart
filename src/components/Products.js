      ...
      {products.map((prod)=>(
        <div key={prod.id} style={{display:'flex',flexDirection:'column',padding:10,border:"1px solid grey",width:"30%",marginTop:10,gap:10}}>
          ...
          {cart.some((p) => p.id === prod.id) ? (
            <button
              style={{padding:5,border:0,borderRadius:5,backgroundColor:'#e53935',color:'white',cursor:"pointer"}}
              onClick={() => dispatch({
            ...
            <button
             style={{padding:5,border:0,borderRadius:5,backgroundColor:'green',color:'white', cursor:"pointer"}}
             onClick={() => dispatch({
                  type: "ADD_TO_CART",
                  payload: {
                    id: prod.id,
